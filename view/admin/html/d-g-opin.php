            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="row">
                            <div class="card">
                                <h5 class="card-header">Opinions List</h5>
                                <div class="table-responsive text-nowrap">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <?php
                                            $Selectionavis = $bdd->prepare("SELECT * FROM avis ORDER BY id");
                                            $Selectionavis->execute();

                                            
                                            while ($avis = $Selectionavis->fetch()) {
                                                $selectuser = $bdd->prepare("SELECT * FROM `users` WHERE id= ?");
                                                $selectuser->execute([$avis["id_user"]]);  
                                                ...
                                                <?php if (isset($_GET['editMode']) && !empty($_GET['editMode']) && isset($_GET['editid']) && !empty($_GET['editid'])) {
                                                    if ($_GET['editid'] == $avis['id']) {
                                                        if (isset($_POST['FormEdit'])) {
                                                            if (!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['contenu']) && !empty($_POST['date'])) {
                                                                
                                                                $nom = htmlspecialchars($_POST['nom']);
                                                                $prenom = htmlspecialchars($_POST['prenom']);
                                                                $numtel = htmlspecialchars($_POST['contenu']);
                                                                $dateres = htmlspecialchars($_POST['date']);

                                                                $update = $bdd->prepare('UPDATE avis SET nom = ?, prenom = ?, contenu = ?, date = ? , WHERE id = ?');
                                                                $update->execute(array($nom, $prenom, $contenu, $date, $avis['id']));
                                                                echo "<script>location.href = './d-g-opin.php'</script>";
                                                            }
                                                        }
                                                        ?>
                                                        <tr>
                                                            <form method="post">
                                                                ...
                                                                <td><input class="form-control" type="text" id="nom" name="nom"
                                                                        value="<?php echo $avis['nom']; ?>" /></td>
                                                                <td><input type="text" class="form-control" id="prenom"
                                                                        name="prenom" value="<?php echo $avis['prenom']; ?>" /></td>
                                                                <td><input type="text" class="form-control" id="contenu" name="contenu"
                                                                        value="<?php echo $avis['contenu']; ?>" /></td>
                                                                <td>
                                                                    <input type="text" class="form-control" id="date" name="date"
                                                                        value="<?php echo $avis['date']; ?>" />
                                                                </td>
