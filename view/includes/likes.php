...
if(isset($_POST['like_post'])){
   ...
   if($user_id != ''){
      ...
      $select_post_like = $bdd->prepare("SELECT * FROM `likes` WHERE post_id = ? AND user_id = ?");
      $select_post_like->execute([$post_id, $user_id]);

      if($select_post_like->rowCount() > 0){
         $remove_like = $bdd->prepare("DELETE FROM `likes` WHERE post_id = ?");
         $remove_like->execute([$post_id]);
         $message[] = 'removed from likes';
      }else{
         $add_like = $bdd->prepare("INSERT INTO `likes`(user_id, post_id, admin_id) VALUES(?,?,?)");
